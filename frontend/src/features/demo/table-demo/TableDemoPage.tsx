...
import type { Person } from "./demo-table-data";
import { createPersonData } from "./demo-table-data";
...
export function TableDemoPage() {
  ...
  const columns = React.useMemo<ColumnDef<Person, unknown>[]>(
    // Fixing the typo from "unkown" to "unknown"
    () => [
      {
        accessorKey: "id",
  ...
  const [data, setData] = React.useState<Person[]>(() =>
    createPersonData(5000),
  );
  const refreshData = () => setData(() => createPersonData(50_000)); //stress test
...
const fuzzySort: SortingFn<unknown> = (rowA, rowB, columnId) => {
  ...
  if (rowA.columnFiltersMeta[columnId] && rowB.columnFiltersMeta[columnId]) {
    const rankDirection = compareItems(
